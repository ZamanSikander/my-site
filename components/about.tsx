        ...
        <MotionSection delay={0.1}>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="rounded border border-ink/10 bg-paper p-4 dark:border-white/10 dark:bg-white/5"
              >
                <p className="text-3xl font-semibold text-clay dark:text-gold">
                  {stat.value}
                </p>
                <p className="mt-1 text-sm leading-6 text-ink/62 dark:text-paper/62">
                  {stat.label}
                </p>
              </div>
            ))}
        ...
        <MotionSection>
          ...
          >
            {experiences.map((item) => (
              <article
                key={item.role}
                className="rounded-md border border-ink/10 bg-paper p-5 dark:border-white/10 dark:bg-ink"
              >
                <p className="text-sm font-semibold uppercase tracking-[0.14em] text-moss dark:text-gold">
                  {item.company}
                </p>
                <h3 className="mt-3 text-xl font-semibold">{item.role}</h3>
                <p className="mt-3 leading-7 text-ink/64 dark:text-paper/64">
                  {item.description}
                </p>
              </article>
            ))}
